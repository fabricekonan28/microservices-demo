...
while ! nc -z users-service.monapp.svc.cluster.local 8001; do
  echo "Waiting for users_service..."
  sleep 2
done

# On attend que products_service soit joignable
while ! nc -z products-service.monapp.svc.cluster.local 8002; do
  echo "Waiting for products_service..."
  sleep 2
done

# On attend que orders_service soit joignable
while ! nc -z orders-service.monapp.svc.cluster.local 8003; do
